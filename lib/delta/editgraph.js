...
(function(exports){
    function Editgraph(a, b, x, y, N, M, dmax) {
        ...
        this.origin = new SnakeHead(this.x-this.y, this.x);
    }
     ...
     * positions.
     */
    Editgraph.prototype.limit = function(left, right) {
        var leftpoint = left.toPoint();
        var rightpoint = right.toPoint();
        this.N = rightpoint.x - leftpoint.x;
        this.M = rightpoint.y - leftpoint.y;
        this.x = leftpoint.x;
        this.y = leftpoint.y;
        this.origin = left.copy();
    }
            ...
            for (k = -d; k <= d; k+=2) {
                ...
                if (delta % 2 === 1 && k >= -d-1+delta && k <= d-1+delta) {
                    if (Vf[k] >= Vb[k]) {
                        ...
                        snake.left = new SnakeHead(this.origin.k + k0, this.origin.x + Vf[k0]);
                        snake.right = head;
                        return d;
                    }
                }
            ...
            for (k = -d+delta; k <= d+delta; k+=2) {
                ...
                if (delta % 2 === 0 && k >= -d && k <= d) {
                    if (Vf[k] >= Vb[k]) {
                        ...
                        snake.left = head;
                        snake.right = new SnakeHead(this.origin.k + k0, this.origin.x + Vb[k0]);
                        return d;
                    }
                }
            }
    ...
    Editgraph.prototype.compute = function(editor) {
        var M = this.M,
            ...
            right = new SnakeHead(this.origin.k + N - M, N),
            d;
        var head, tail;
