...
(function(exports){
    ...
    Editgraph.prototype.forEachDpath = function (dmax, onpath) {
        ...
        for (d = 0; d <= dmax; d++) {
            for (k = -d; k <= d; k+=2) {
                var result = this.extendDpathFw(d, k, V, 0, 0, N, M, function(x, x0, k0) {
                    var y = x-k;
    ...
    Editgraph.prototype.forEachReverseDpath = function (dmax, onpath) {
        ...
        for (d = 0; d <= dmax; d++) {
            for (k = -d; k <= d; k+=2) {
                result = this.extendDpathBw(d+delta, k, V, 0, 0, N, M, function(x, x0, k0) {
                    var y = x-k;
    ...
    Editgraph.prototype.extendDpathFw = function(d, k, V, dx, dy, N, M, onpath) {
        var k0, x0, x, y;
        ...
        // follow the diagonal
        while (x < N && y < M && this.equals(this.a[dx + x], this.b[dy + y])) {
            x++;
            y++;
        }
        ...
        if (onpath)
            return onpath(dx + x, dx + x0, k0);
    };


    Editgraph.prototype.extendDpathBw = function(d, k, V, dx, dy, N, M, onpath) {
        var k0, x0, x, y;
        ...
        // follow the diagonal
        while (x > 0 && y > 0 && this.equals(this.a[dx + x], this.b[dy + y])) {
            x--;
            y--;
        }
     ...
     * Middle snake
     */
    Editgraph.prototype.middleSnake = function (dxf, dyf, dxb, dyb, N, M, onpath) {
        var dmax = N+M;
        var delta = N-M;
        var d, k;
        var Vf = {'1': 0};
        ...
        for (d = 0; d <= dmax/2; d++) {
            for (k = -d; k <= d; k+=2) {
                result = this.extendDpathFw(d, k, Vf, dxf, dyf, N, M, function(x, x0, k0) {
                    var y = x-k;
                    var y0 = x0-k;
                    ...
                    if (delta % 2 === 1 && (k === delta - (d-1) || k === delta + (d+1))) {
                        if (Vf[k] >= Vb[k]) {
                            ...
                            return [dxf + x, dyf + y, dxf + x0, dyf + y0]
                        }
                    }
                });
                if (result) {
            ...
            for (k = -d; k <= d; k+=2) {
                result = this.extendDpathBw(d, k+delta, Vb, dxb, dyb, N, M, function(d, x, k, x0, k0) {
                    var y = x-k;
                    var y0 = x0-k;
                    ...
                    if (delta % 2 === 0 && (k + delta === -d || k + delta === d)) {
                        if (Vf[k+delta] >= Vb[k+delta]) {
                            // return last forward snake
                            return [dxb + x, dyb + y, dxb + x0, dyb + y0]
                        }
                    }
                });
                if (result) {
