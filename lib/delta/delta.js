...
Delta.prototype.attach = function(resolver, handlerfactory) {
    var fails = 0, i, start, op, nodes, depth, handler;
 ...
 */
Delta.prototype.collect = function(node, matching, contextgen, updater, path, lastmatch) {
    ...
    parambuf = new ParameterBuffer(function(removes, inserts) {
        var head = contextgen.head(lastmatch);
        var tail = contextgen.tail(removes.length ? removes[removes.length-1] : lastmatch);
    ...
    while (a_nodes[i] || b_nodes[k]) {
        ...
        if (a && !matching.get(a)) {
            ...
            // Recurse
            lastmatch = this.collect(a, matching, contextgen, updater, path, lastmatch);
