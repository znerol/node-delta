...
Delta.prototype.attach = function(resolver, handlerfactory) {
    ...
    for (i = 0; i < this.operations.length; i++) {
        ...
        if (op.oldvalue.length && !nodes[depth]) {
...
Delta.prototype.collect = function(node, matching, contextgen, updater, path) {
    ...
    updater && updater(node, function(a, b) {
        ...
        var op = new Operation(UPDATE_NODE_TYPE, path.slice(), head, tail, [a], [b]);
        this.add(op);
    }, this);
    ...
    parambuf = new ParameterBuffer(function(removes, inserts) {
        ...
        var op = new Operation(UPDATE_FOREST_TYPE, path.slice(), head, tail, removes, inserts);
        this.add(op);
    }, this);
