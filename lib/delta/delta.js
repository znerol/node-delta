...
(function(exports, tree) {
     ...
     */
    function ItemAggregator(anchor, callback, T) {
        this.anchor = anchor;
     ...
     * afterwards.
     */
    ItemAggregator.prototype.flush = function(node) {
        if (this.items.length > 0) {
            this.callback.call(this.T, this.anchor, this.items);
            this.items = [];
        }
        if (node) {
            this.anchor = node;
    ...
    Delta.prototype.collect = function(node, matching, contextgen, updater, path) {
        var inserter, remover, i, k, l, a_nodes, b_nodes, a, b;
        ...
        inserter = new ItemAggregator(node, function(anchor, inserts) {
        ...
        remover = new ItemAggregator(node, function(anchor, removes) {
            var head = contextgen.head(anchor);
            var tail = contextgen.tail(anchor, removes.length);
        ...
        while ((a = a_nodes[i]) || (b = b_nodes[k])) {
            if (a && !matching.get(a)) {
                ...
                inserter.flush(a);
                remover.flush(a); // No typo, a is last context node.

                // Recurse
                this.collect(a, matching, contextgen, updater, path);
                i++;
            }
            else if (b && matching.get(b)) {
                k++;
    ...
    function ContextGenerator(radius, nodeindex, valindex) {
        this.head = function(next, offset) {
            var i, result = [];
            for (i = offset - radius; i < offset; i++) {
                result.push(valindex.get(nodeindex.get(next, i)));
            }
            return result;
        }

        this.tail = function(first, offset) {
            var i, result = [];
            for (i = offset; i < offset + radius; i++) {
                result.push(valindex.get(nodeindex.get(first, i)));
            }
            return result;
        }
    }
    ...
    typeof exports === 'undefined' ? (DeltaJS.delta={}) : exports,
