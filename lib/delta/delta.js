...
(function(exports, tree) {
    ...
    function FingerprintFactory(index, radius) {
        this.r = radius;
        this.index = index;
    }
     ...
     */
    FingerprintFactory.prototype.fingerprint = function(anchor, length, skipbody) {
        var i, n, fp = {
            'body':[],
            'head':[],
            'tail':[]
        ...
        if (skipbody) {
            ...
            while (i < length) {
                n = this.index.get(anchor, i++);
                fp.body.push(n && n.value);
            }
        }

        while (i < length + this.r) {
            n = this.index.get(anchor, i++);
            fp.tail.push(n && n.value);
        }
    ...
    Editor.prototype.insert = function(path, b_nodes) {
        var fp = this.fpfactory.fingerprint(b_nodes[0], b_nodes.length, true);
