...
Delta.prototype.attach = function(resolver, handlerfactory) {
    var fails = 0, i = 0, start, anchor,
        attacher = new DeltaAttacher(resolver, handlerfactory);

    attacher.forEachOperation(this, function(res, op) {
        if (!res.anchor || res.tail.length > 0) {
            // Resolving failed completely
            // FIXME: error message? Resolver could store one into res.
            fails++;
        }
        else {
            try {
                this.handlers[i] =
                    handlerfactory.createOperationHandler(res.anchor, op.type,
                        op.path, op.remove, op.insert);
            }
            catch (err) {
                // FIXME: save error message for later retrieval.
                fails++
            }
...
function DeltaAttacher(resolver) {
    this.resolver = resolver;
}

DeltaAttacher.prototype.forEachOperation = function(delta, callback, T) {
    var i, op, res;
...
Delta.prototype.collect = function(node, matching, contextgen, updater) {
    ...
    var operationfactory = new OperationFactory(contextgen);
