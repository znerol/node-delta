 ...
 * @param start     First node which should be removed. Should point to the
 *                  node before which elements should be inserted if no nodes
 *                  are to be removed.
...
DOMOperationHandlerFactory.prototype.createForestUpdateOperationHandler = function(
    ...
    if (!start && length) {
        throw new Error('Remove operation requires a start point');
    }

    if (length) {
        for (i = start.childidx; i < start.childidx + length; i++) {
            oldnodes.push(node.children[i].data);
        }
        before = oldnodes[oldnodes.length - 1].nextSibling;
    }
    else {
        before = start && start.data;
    }
