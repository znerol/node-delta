...
(function(exports, tree, platform) {
     ...
     */
    function DOMNodeAttributeOperation(oldnode, newnode, invert) {
        ...
        this.oldattrs = invert ? newattrs : oldattrs;
        this.newattrs = invert ? oldattrs : newattrs;
     ...
     *                      subtrees
     */
    function DOMTreeSequenceOperation(node, before, oldnodes, newnodes, invert) {
        this.node = node;
        this.before = before;

        this.oldnodes = invert ? newnodes : oldnodes;
        this.newnodes = invert ? oldnodes : newnodes;
     ...
     */
    DOMOperationFactory.prototype.createNodeUpdateOperation = function(node, oldvalue, newvalue, invert) {
        newvalue = node.data.ownerDocument.importNode(newvalue.firstChild, true);
        return new DOMNodeAttributeOperation(node, oldvalue, newvalue, invert);
    };
     ...
     *
     * @param node      A DeltaJS.tree.node
     * @param before    Index of the child node befor which the new nodes will
     *                  be inserted.
     * @param newvalue  Single DOM element with the new subtrees as children.
     */
    DOMOperationFactory.prototype.createSubtreeInsertOperation = function(node, before, newvalue, invert) {
        ...
        if (newvalue.hasChildNodes()) {
            ...
            return new DOMTreeSequenceOperation(node.data, before, [], newnodes, invert);
        }
    };
     ...
     *
     * @param node      A DeltaJS.tree.node
     * @param oldvalue  Single DOM element with the new subtrees as children.
     */
    DOMOperationFactory.prototype.createSubtreeRemoveOperation = function(node, oldvalue, invert) {
        var oldnodes, before;
        if (oldvalue.hasChildNodes()) {
            oldnodes = Array.prototype.slice.call(oldvalue.childNodes);
            before = oldnodes[oldnodes.length-1].nextSibling;

            return new DOMTreeSequenceOperation(node.data, before, oldnodes, [], invert);
        }
    };
