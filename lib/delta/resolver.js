...
(function(exports, tree) {
    ...
    UniformDepthResolver.prototype.find = function(path, values, head, tail) {
        var node, result, noff = 0;

        node = this.resolveExact(path);

        if (!node) {
            node = this.resolveNextInGeneration(path);
            noff = 1;
        }
        if (!node) {
            node = this.genindex.last(path.length);
        }

        if (node) {
            this.matcher.setPattern(values, head, tail);
            for (i = -this.r + noff; i <= this.r + noff; i++) {
                var f = this.matcher.matchQuality(i, function(offset, value) {
                    var cand = this.genindex.get(node, offset);
                    return cand && cand.value = value;
                }, this);

                if (f > q && f >= this.t) {
                    q = f;
                    result = this.genindex.get(node, i);
                }
            }
     ...
     */
    UniformDepthResolver.prototype.resolveExact = function(path) {
        var i, node = this.refnode;

        for (i = 0; i < path.length && node; i++) {
     ...
     */
    UniformDepthResolver.prototype.resolveNextInGeneration = function(path, start) {
     ...
     */
    WeightedContextMatcher.prototype.setPattern = function(values, head, tail) {
        this.values = values;
        this.head = head || [];
        this.tail = tail || [];
    }
    ...
    WeightedContextMatcher.prototype.matchQuality = function(offset, test, T)
    {
        ...
        for (i = 0, k = offset; i < n; i++, k++) {
            if (!test.call(T, k, this.values[i])) {
                return 0;
            }
        }
        ...
        if (this.qf.length && (this.head.length || this.tail.length)) {
            ...
            for (i = 0, k = offset + this.values.length; i < n; i++, k++) {
