...
ContextResolver.prototype.find = function(path, body, head, tail, type) {
    var nodes, node, siblings, lastsib, i, q = 0, f, best, bestnode, result,
        flatbody;
    ...
    // top-down resolver did not came through to the very last path component.
    if (nodes.length === path.length) {
        siblings = nodes[path.length-1].children;
        lastsib = siblings[siblings.length-1];
        if (lastsib) {
            nodes.push(this.nodeindex.get(lastsib,
