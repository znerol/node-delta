...
(function(exports, tree) {
    ...
    UniformDepthResolver.prototype.find = function(path, values, head, tail) {
        ...
        best = resolveByContext(best, generation, values, head, tail);
        return best;
    };
    ...
    UniformDepthResolver.prototype.resolveByContext = function(best,
            generation, values, head, tail) {
        var w = new ContextWindow(generation, this.radius);

        w.forEach(best.index, values.length, head.length, tail.length, function(
                    cand_values, cand_head, cand_tail) {
            var i;
            for (i = 0; i < cand_values; i++) {
                // Do not consider locations where values are not equal
                if (cand_values[i] !== values[i]) {
                    return;
                }
            }
     ...
     */
    function ContextWindow(content, radius) {
        this.radius = radius;
     ...
     */
    ContextWindow.prototype.forEach = function(center, length, head, tail,
            callback, T) {
        var fpsize = head + length + tail,
            start = center - head,
            end = start + length + this.radius,
            i, k;

        if (!length) {
            if (!head) {
                throw new Error('Either length or head must be non-zero');
            }
        ...
        for (i = start - this.radius; i < end; i++) {
            fp = [];
            ...
            Array.prototype.push.apply(fp, this.content.slice(k, i + fpsize));

            for (k = i + fpsize + this.content.length; k > 0; k--) {
                fp.push(undefined);
            }
