...
(function(exports, tree) {
     ...
     */
    function UniformDepthResolver(refnode, radius, threshold, nodeindex, matcher) {
    ...
    /**
     * Compare a document node against a content node from the patch.
     */
    UniformDepthResolver.prototype.equalContent = function(docnode, patchnode) {
        return docnode === undefined ? patchnode === undefined : 
            docnode && patchnode && docnode.value === patchnode.value;
    }


    /**
     * Compare a document node against a context node value.
     */
    UniformDepthResolver.prototype.equalContext = function(docnode, value) {
     ...
     */
    UniformDepthResolver.prototype.find = function(path, body, head, tail) {
        var nodes, node, q = 0, f, bestnode, result;
        ...
        if (node) {
            ...
            for (i = -this.r; i <= this.r; i++) {
                f = this.matcher.matchContent(i, function(offset, patchnode) {
                    var cand = this.nodeindex.get(node, offset);
                    return this.equalContent(cand, patchnode);
                }, this);
                if (f !== 1) {
                    continue;
                }
                ...
                f = this.matcher.matchContext(i, function(offset, value) {
                    var cand = this.nodeindex.get(node, offset);
                    return this.equalContext(cand, value);
                }, this);
                if (f > q && f >= this.t) {
                    q = f;
