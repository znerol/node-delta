...
ContextResolver.prototype.find = function(path, body, head, tail, type) {
    ...
    if (path.length === 0) {
        ...
        siblings = guess.anchor.base.children;
        if (guess.anchor.index < 0) {
            ...
            node = guess.anchor.base;
        }
        else if (guess.anchor.index < siblings.length) {
            // Start with the appointed child node
            node = siblings[guess.anchor.index];
        }
        else {
