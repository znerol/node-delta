...
(function(exports, lcs) {
    ...
    Diff.prototype.matchLeafLCS = function(matching) {
        var a_leaves = [],
            b_leaves = [],
            lcsequals = this.options.lcsEqualCallback;
            lcsinst = new lcs.LCS(a_leaves, b_leaves);

        // Augment LCS instance. Leaves are considered equal if their values
        // match and if they have the same tree depth. Also take the optional
        // user supplied callback into account.
        if (lcsequals) {
            lcsinst.equals = function(a, b) {
                return a.value === b.value && a.depth === b.depth && lcsequals(a, b);
            };
        }
        else {
            lcsinst.equals = function(a, b) {
